...
def scan_http_server(domain):
    ...
    try:
        # inspired by project 2 and textbook
        conn.request("GET", "/")
        response = conn.getresponse()
...
def scan_redirect_to_https(domain):
    url = "http://" + domain
    try:
        # implementation inspired by https://requests.readthedocs.io/en/latest/user/advanced/
...
def scan_rdns_names(ipv4_addresses):
    ...
    for address in ipv4_addresses:
        try:
            ...
            print(f"nslookup command timed out for {ip_pattern}")
        except Exception as e:
            print(f"An error occurred: {e}")
            continue
    return list(set(results))
